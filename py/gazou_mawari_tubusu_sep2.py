...
if len(sys.argv) < 4:
	sys.stderr.write("python gazou_mawari_tubusu_sep2.py input_img mask_img output_img\n")
	sys.exit(1)
...
nextImg = copy.deepcopy(img)
nextValid = copy.deepcopy(validFlag)

while len(nextWork) > 0:
	currentWork = nextWork
	nextWork = []
	for y, x in currentWork:
		if nElem > 0:
